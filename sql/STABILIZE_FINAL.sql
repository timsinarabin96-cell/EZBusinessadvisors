-- ============================================================
-- CONCORD DEAL PLATFORM - SCHEMA STABILIZATION PATCH (FINAL)
-- Generated from project sql/ schema files + verified against live DB.
-- 100% SAFE: only CREATE TABLE IF NOT EXISTS + ENABLE RLS.
-- NO DROP, NO TRUNCATE, NO DELETE. Runs in a single DO $$ transaction:
...
-- documents (Document Builder core)
...
-- broker_photos / certified_brokers / profile_images (no repo schema - minimal auto-defined)

-- broker_photos (business card upload; no schema def in repo - minimal safe)
create table if not exists public.broker_photos (
  id            uuid primary key default gen_random_uuid(),
  broker_id     uuid references public.profiles(id) on delete cascade,
  url           text,
  uploaded_at   timestamptz not null default now()
);
create table if not exists public.certified_brokers (
  id            uuid primary key default gen_random_uuid(),
  profile_id    uuid references public.profiles(id) on delete cascade,
...
create table if not exists public.profile_images (
  ...
  ALTER TABLE public.certified_brokers ENABLE ROW LEVEL SECURITY;
