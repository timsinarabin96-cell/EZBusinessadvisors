...
// (margin quality, industry). Estimates and their inputs/multiples are stored
// as jsonb so the breakdown can be re-explained. Server-only. Never throws.
// =============================================================================
...
export function computeValuation(listing: ValuationInputs & { id?: string }): Omit<ValuationResult, 'id'> | null {
  ...
  if (sde <= 0 && revenue <= 0) return null

  const band = multipleForIndustry(listing.industry)
  ...
  const sdeMin = sde * minMult
  const sdeMax = sde * maxMult
  const revMin = revenue * 0.8
  const revMax = revenue * 1.2

  // Blend SDE-multiple with the revenue cross-check (50/50) so both methods
  // inform the range; a missing metric simply falls out of the average.
  let min = 0
  let max = 0
  let count = 0
  if (sde > 0) {
  ...
  return {
    ...
    method: 'SDE multiple (2.5-3.5x, industry-adjusted) cross-checked against 0.8-1.2x revenue',
    multiples: {
      industry: band.label,
...
export async function estimateValuation(listingId: string): Promise<{ ok: boolean; error?: string; estimate?: ValuationResult }> {
  try {
    ...
    const { data: listing } = await svc
      .from('listings')
      .select('id, agency_id, business_name, industry, annual_revenue, sde, asking_price')
      .eq('id', listingId)
      .maybeSingle()
