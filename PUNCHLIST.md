...
| financial_documents METADATA RLS (09-01) | ✅ Verified | Yavin | Was `fd_select … using (true)` — ANY authenticated user could read every financial_documents row (file names, paths, listing/deal ids, recast/BOV/CIM data). Now agency/NDA-scoped: uploader OR owning-agency member OR NDA-approved buyer (visible_to_buyer docs only). Insert/update/delete scoped to owning agency too. Live probe `verify-fd-rls.cjs` 9/9: QA member sees docs, cross-agency Harbor agent sees 0, no-NDA buyer sees 0, NDA-approved buyer sees shared doc only, anonymous sees 0, cross-agency INSERT/UPDATE/DELETE blocked. Migration: `sql/harden_financial_documents_metadata_rls.sql`. |
| Claude prompt-injection hardening (09-01) | ✅ Verified | Yavin | Document-extraction + photo-analysis prompts now mark uploaded content as UNTRUSTED DATA (never follow embedded instructions) + delimit content (`<document_content_begin>`); regression test `tests/promptInjectionGuard.test.mts` 4/4. Env-gated ZDR opt-in (`ANTHROPIC_ZDR=true` → `x-api-zdr` header) added to Claude client, default OFF. |
