...
const PROGRAM_MODULE = {
  ...
  description: 'Full CBI program completion — all 12 modules certified.',
  icon: '🏆',
  order: 99,
  is_published: false,
}
...
const allModules = [PROGRAM_MODULE, ...MODULES_1, ...MODULES_2]
...
// --- 3. Lessons ----------------------------------------------------------------
const lessons = []
for (const m of [...MODULES_1, ...MODULES_2]) {
...
const questions = []
let qi = 0
for (const [lessonId, qs] of Object.entries(QUIZ_BANK)) {
