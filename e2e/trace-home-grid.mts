...
const info = await page.evaluate(() => {
  const grids = [...document.querySelectorAll('div')].filter((d) => {
    ...
    return cs.display === 'grid' && cs.gridTemplateColumns.includes('minmax')
  })
  return grids.map((g) => {
    ...
    return {
      ...
      firstChildMinW: fcs ? fcs.minWidth : 'none',
      firstChildW: first ? Math.round(first.getBoundingClientRect().width) : 0,
    }
  })
})
