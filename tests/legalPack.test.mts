...
test('key agreements are properly sectioned like real broker packages', () => {
  ...
  // Full commission protection: liquidated-damages triggers + commission structure option
  assert.match(marketing, /liquidated damages/, 'Marketing lacks liquidated-damages commission protection')
  assert.match(marketing, /commission_clause/, 'Marketing lacks commission structure option')
  assert.match(marketing, /ready, willing and able/, 'Marketing lacks ready-willing-able trigger')
  assert.match(marketing, /stock or interest in the Business/, 'Marketing lacks stock-transfer trigger')
  assert.match(marketing, /PRINCIPAL GUARANTY/, 'Marketing lacks principal guaranty')
  assert.match(marketing, /JURY WAIVER/, 'Marketing lacks jury waiver')
  assert.match(listing, /liquidated damages/, 'Listing lacks liquidated-damages commission protection')
  assert.match(listing, /commission_clause/, 'Listing lacks commission structure option')
  assert.match(listing, /PRINCIPAL GUARANTY/, 'Listing lacks principal guaranty')
  assert.match(listing, /JURY WAIVER/, 'Listing lacks jury waiver')
  assert.match(listing, /COMMISSION AND COMPENSATION/, 'Listing lacks commission-and-compensation section')
