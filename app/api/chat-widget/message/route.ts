...
export async function POST(req: NextRequest) {
  ...
  // Now the assistant answers here, in-process, via DeepSeek (already
  // configured), and the reply is stored in the same thread so the widget's
  // existing /api/chat-widget/poll picks it up unchanged.
  try {
    ...
    if (isDeepSeekConfigured()) {
      ...
      const { text } = await chatWithDeepSeek({ system, userMessage: message, maxTokens: 400 })
