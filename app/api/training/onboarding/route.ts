...
export async function GET(req: NextRequest) {
  ...
  const [{ data: modules }, { data: tasks }] = await Promise.all([
    db.from('agency_training_modules').select('id, title, description, lesson_content, quiz_question, quiz_options, order').eq('program_id', enrollment.program_id).order('order'),
  ...
  return NextResponse.json({
    ...
    modules: (modules || []).map((module: any) => ({ ...module, task: taskByModule.get(module.id) || null })),
  })
}
...
export async function POST(req: NextRequest) {
  ...
  const { data: module } = await db.from('agency_training_modules').select('id, quiz_correct_answer').eq('id', moduleId).eq('program_id', enrollment.program_id).maybeSingle()
  ...
  const { count } = await db.from('agency_training_tasks').select('id', { count: 'exact', head: true }).eq('enrollment_id', enrollment.id).eq('completed', false)
