          ...
          {escrow.length ? <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(260px,1fr))', gap: 14 }}>
            {escrow.map((account) => {
              ...
              return <div key={account.id} style={{ border: '1px solid rgba(15,23,42,.08)', borderRadius: 17, padding: 16, background: 'linear-gradient(145deg,#fff,#f8fafc)' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center' }}>{steps.map((step, index) => { const current = steps.indexOf(account.status); const done = index <= current; return <div key={step} style={{ display: 'flex', alignItems: 'center', flex: 1 }}><button type="button" onClick={() => index === current + 1 && setEscrowStatus(account, step)} disabled={index > current + 1 || index <= current} aria-label={`Mark ${step}`} style={{ width: 25, height: 25, borderRadius: 999, border: 0, background: done ? '#0f3460' : '#e2e8f0', color: '#fff', fontSize: 11, fontWeight: 900, cursor: index === current + 1 ? 'pointer' : 'default' }}>{done ? '✓' : index + 1}</button>{index < steps.length - 1 && <span style={{ height: 2, flex: 1, background: index < current ? '#0f3460' : '#e2e8f0' }} />}</div> })}</div>
