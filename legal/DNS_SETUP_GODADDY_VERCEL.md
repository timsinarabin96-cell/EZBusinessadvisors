...
**Detected:** 2026-08-26 · **Issue:** `concorddeal.com` returns NXDOMAIN (domain doesn't exist on the internet). The site is reachable at `concorddeal.com` but NOT at the branded address. This blocks launch AND the Stripe webhook. **~5 minutes.**

---

## Step 1 — Add the CNAME at GoDaddy

1. Log in at **godaddy.com** → **Domain Portfolio** → `concorddeal.com` → **DNS** (Manage DNS)
...
2. If `concorddeal.com` is listed as a domain:
...
3. If it's NOT listed → click **Add Domain** → enter `concorddeal.com` → follow the prompts.

## Step 3 — Verify DNS propagated

Wait ~5–10 minutes, then check from a normal browser (or your phone, not on Wi-Fi):

- Open `https://concorddeal.com` — you should see the marketplace homepage.
- Or check DNS: visit `https://dns.google/resolve?name=concorddeal.com&type=CNAME` — you want a `Status: 0` with a CNAME answer pointing at `cname.vercel-dns.com`.
...
- [ ] `https://concorddeal.com` loads in a browser
...
*If the domain still doesn't resolve ~30 min after the CNAME, check GoDaddy DNS again (record saved? host spelled `concord`, not `concorddeal.com`?) — or DM me and I'll re-check propagation live.*
